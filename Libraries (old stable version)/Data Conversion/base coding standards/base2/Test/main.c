...
// размер массива для выходного текста Base2
//
// -------------------------------------------------------------------------------
// size of the array for the Base2 output text
//
#define BASEX_ARRAY_SIZE (DATA_ARRAY_SIZE * 8) // overhead = 700%





// определение массивов данных и текстового массива Base2
//
// -------------------------------------------------------------------------------
// defining Base2 data arrays and text arrays
//
...
void Run_Test()
{
	...
	for (uint32_t i = 0; i < TEST_NUM_ITERATIONS; ++i)
	{
		...
		uint32_t basex_encoded_array_size = Base2_Encode(data_array1, data_size, basex_array);

		uint32_t basex_decoded_array_size = Base2_Decode(basex_array, basex_encoded_array_size, data_array2);
