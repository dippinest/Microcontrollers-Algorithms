...
int main(void)
{
	...
	// данного ГСПЧ нулевым значением. В данном примере в качестве начального
	// значения генератора (seed) был взят шум АЦП,
	// сложенный с константой по модулю 2^32
	//
	...
	// the ADC noise, added with a constant modulo 2^32, was taken as the initial value
	// of the generator (seed).
	//
	// Attention! Make sure that the initial value (seed)
	// is not zero, otherwise the generator will not work!
	//
	XORShift32_t generator = XORShift32_Create_Object( ADC_Get_Random_Entropy_Value_32bit(0) | 0x1 );
	...
	while (1)
	{
		...
		//
		int32_t random_number = XORShift32_Get_Int32_Value(&generator, -100, 100);
