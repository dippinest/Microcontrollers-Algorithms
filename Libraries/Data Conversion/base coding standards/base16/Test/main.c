...
// размер массива для выходного текста Base16
//
// -------------------------------------------------------------------------------
// size of the array for the Base16 output text
//
#define BASEX_ARRAY_SIZE (DATA_ARRAY_SIZE * 2) // overhead = 100%





// определение массивов данных и текстового массива Base16
//
// -------------------------------------------------------------------------------
// defining Base16 data arrays and text arrays
//
...
void Run_Test()
{
	...
	for (uint32_t i = 0; i < TEST_NUM_ITERATIONS; ++i)
	{
		...
		uint32_t basex_encoded_array_size = Base16_Encode(data_array1, data_size, basex_array);

		uint32_t basex_decoded_array_size = Base16_Decode(basex_array, basex_encoded_array_size, data_array2);
