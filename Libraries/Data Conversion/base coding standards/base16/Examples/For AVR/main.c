...
//
char encoded_data_buf[DATA_SIZE * 2]; // // overhead = 100%
...
int main(void)
{
	...
	uint32_t encoded_data_size = Base16_Encode(data, strlen(data), encoded_data_buf);

	UART_StringFmt_Transmit("\r\n\r\n\r\nEncoded data:\r\n\r\n"); UART_String_Transmit(encoded_data_buf);


	uint32_t decoded_data_size = Base16_Decode(encoded_data_buf, encoded_data_size, decoded_data_buf);
