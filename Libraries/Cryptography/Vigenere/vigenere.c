...
uint16_t Vigenere_Standart_Stream_Encrypt(void* alphabet, uint8_t alphabet_size, void* data, uint16_t data_size, Vigenere_Stream_Data_t *v_stream)
{
	...
	int16_t data_char_pos, key_char_pos = 0;
	...
	for (i = 0; i < data_size; ++i)
	{
		...
		key_char_pos = _Vigenere_Find_Index_Of_First_Occurrence_Of_A_Byte(alphabet, alphabet_size, ((char*)(v_stream->key))[(i + (v_stream->key_pos_counter)) % (v_stream->key_size)]);
	...
	v_stream->key_pos_counter = key_char_pos;
...
uint16_t Vigenere_Bytes_Stream_Encrypt(void* data, uint16_t data_size, Vigenere_Stream_Data_t *v_stream)
{
	...
	for (; i < data_size; ++i)
	{
		new_data_char_pos = i + (v_stream->key_pos_counter);
		
		((uint8_t*)data)[i] += ((uint8_t*)(v_stream->key))[new_data_char_pos % (v_stream->key_size)];
	}
	
	
	v_stream->key_pos_counter = new_data_char_pos;
