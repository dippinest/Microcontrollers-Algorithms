...
int main(void)
{
	...
	uint16_t encrypt_data_size = RTEA256BitKey_Encrypt_OFB(initial_vector_64bit, data, DATA_SIZE, key);
	UART_StringFmt_Transmit("\r\n\r\nEncrypt data: "); print_data(data, encrypt_data_size);
	
	uint16_t decrypt_data_size = RTEA256BitKey_Decrypt_OFB(initial_vector_64bit, data, DATA_SIZE, key);
