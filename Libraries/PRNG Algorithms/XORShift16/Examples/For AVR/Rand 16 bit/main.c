...
int main(void)
{
	...
	// сложенный с константой по модулю 2^16
	//
	...
	// the ADC noise, added with a constant modulo 2^16, was taken as the initial value
	// of the generator (seed).
	//
	// Attention! Make sure that the initial value (seed)
	// is not zero, otherwise the generator will not work!
	//
	XORShift16_t generator = XORShift16_Create_Object( ADC_Get_Random_Entropy_Value_16bit(0) | 0x1 );
	...
	while (1)
	{
		...
		//
		int16_t random_number = XORShift16_Get_Int16_Value(&generator, -100, 100);
