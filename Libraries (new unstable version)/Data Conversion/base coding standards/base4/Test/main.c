...
// размер массива для выходного текста Base4
//
// -------------------------------------------------------------------------------
// size of the array for the Base4 output text
//
#define BASEX_ARRAY_SIZE (DATA_ARRAY_SIZE * 3) // overhead = 200%





// определение массивов данных и текстового массива Base4
//
// -------------------------------------------------------------------------------
// defining Base4 data arrays and text arrays
//
...
void Run_Test()
{
	...
	for (uint32_t i = 0; i < TEST_NUM_ITERATIONS; ++i)
	{
		...
		uint32_t basex_encoded_array_size = Base4_Encode(data_array1, data_size, basex_array);

		uint32_t basex_decoded_array_size = Base4_Decode(basex_array, basex_encoded_array_size, data_array2);
