...
// объект для хранения цвета в RGB
//
// -------------------------------------------------------------------------------
// object for storing colors in RGB
//
RGB_t rgb;


// объект для хранения цвета в HSV.
// Инициализируем его так, чтобы при инкременте составляющей 'h'
// происходило вращение по цветовому кругу HSV (эффект радуги)
//
// -------------------------------------------------------------------------------
// object for storing colors in HSV.
// Initialize it so that when the 'h' component is incremented,
// rotation occurs along the HSV color circle (rainbow effect)
//
...
int main(void)
{
	...
	while (1)
	{
		...
		//
		rgb = Color_Transform_HSV_To_RGB(hsv);
