...
int main(void)
{
	...
	while (1)
	{
		...
		UART_String_Transmit("1 Temp (*K) "); UART_StringLn_Transmit(FTOA_Float32_To_String(string_buffer, temp_to_kelvin_1,     3, 1, '.'));
		UART_String_Transmit("1 Temp (*C) "); UART_StringLn_Transmit(FTOA_Float32_To_String(string_buffer, temp_to_celsius_1,    3, 1, '.'));
		UART_String_Transmit("1 Temp (*F) "); UART_StringLn_Transmit(FTOA_Float32_To_String(string_buffer, temp_to_fahrenheit_1, 3, 1, '.'));
		UART_NEW_LINE;
		
		
		UART_String_Transmit("2 Temp (*K) "); UART_StringLn_Transmit(FTOA_Float32_To_String(string_buffer, temp_to_kelvin_2,     3, 1, '.'));
		UART_String_Transmit("2 Temp (*C) "); UART_StringLn_Transmit(FTOA_Float32_To_String(string_buffer, temp_to_celsius_2,    3, 1, '.'));
		UART_String_Transmit("2 Temp (*F) "); UART_StringLn_Transmit(FTOA_Float32_To_String(string_buffer, temp_to_fahrenheit_2, 3, 1, '.'));
		UART_NEW_LINE;
